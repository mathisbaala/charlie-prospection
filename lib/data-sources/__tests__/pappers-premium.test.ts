...
describe('getPappersTokenStatus', () => {
  afterEach(() => {
    vi.unstubAllEnvs()
    vi.resetModules()
    vi.doUnmock('@/lib/observability/logger')
  ...
  it('parses /suivi-jetons response and computes remaining', async () => {
    vi.doMock('@/lib/observability/logger', () => ({
      timedFetch: async () =>
        ({
          ok: true,
          json: async () => ({
            jetons_abonnement: 500,
            jetons_abonnement_utilises: 228.9,
            jetons_pay_as_you_go_restants: 50,
          }),
        }) as unknown as Response,
    }))
    vi.stubEnv('PAPPERS_API_KEY', 'fake')

    const { getPappersTokenStatus } = await import('../pappers')
    const status = await getPappersTokenStatus()

    expect(status).not.toBeNull()
    expect(status?.jetons_abonnement).toBe(500)
    expect(status?.jetons_abonnement_utilises).toBeCloseTo(228.9)
    expect(status?.remaining).toBeCloseTo(321.1)
  })

  it('returns null on fetch error (instrumentation must not crash callers)', async () => {
    vi.doMock('@/lib/observability/logger', () => ({
      timedFetch: async () => {
    ...
    const { getPappersTokenStatus } = await import('../pappers')
    expect(await getPappersTokenStatus()).toBeNull()
  })
})
